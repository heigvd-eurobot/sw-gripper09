 ...
 * @svn     $Id: i2c.c 500 2008-11-26 20:49:33Z yves $
 */
...
void I2C_Init(void)
{
	...
	// I2C Addresses
	I2caRegs.I2COAR = conf.env.i2c_addr; // Our Slave address
	I2caRegs.I2CSAR = 0x01;				 // Colibri address
...
// Read memory
// @param compteur de bytes (num�ro du byte � envoyer)
...
void write_memory(char *data, int length)
{
  ...
  // Adaptation des gains des r�gulateurs moteur A
  ...
  // Adaptation des gains des r�gulateurs moteur B
...
// NOTICE: Les tests effectu�s ont d�tect� un probl�me lors d'une lecture
// tr�s soutenue. Le module se bloque. Il se peut qu'il s'agisse d'un probl�me
// de stackoverflow. Aussi, le mode polling est pr�f�rable dans cette application
// A titre d'information cette function reste pr�sente dans ce fichier
...
i2c_int1a_isr (void)
{
	...
	// Le chip est adress� cela peut dire que
	// On re�ois un nouveau message
	// On poursuit un message existant auquel cas
	// On dois d�j� avoir re�u un XRDY
	if(int_src == I2C_AAS)
	{
	...
	// Un signal de stop est d�tect�
	// Dans tous les cas la transmission se termine. 
	// La donn�e est �crite � condition que l'on aie re�u quelque chose
	...
	// On re�ois un byte par cons�quent
	...
	// Il faut pr�parer une donn�e pour l'emission
	...
	// La donn�e n'a pas �t� transmise correctement, on reset la transmission
...
// Initialize i2c.
// Cette fonction initialise le bus I2C sur le TMS2806, elle doit �tre appel�e
