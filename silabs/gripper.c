/*============================================================\
 ...
 ============================================================
   ...
   Est-il vraiement nécessaire de tjr faire le start moteur?
   en cas d'obstacle détecté, comment je sais la position actuel? nécessaire?
...
void Homing_Mott()
{
   ...
   if(homing==1)  // le homing des bras et du barilet a réussi.
   {
...
{ 
   ...
   // initialise les machines d'états
...
void  State_machine_bg()
{
   ...
   if(state_changed||(memo_state!=pos_actu.state_bg))      // test si une irq a changé l'état
      state_changed=1;
   else
      state_changed=0;
   ...
   switch(pos_actu.state_bg)
   {
      ...
      case SBG_PRISE_PAL_1:
         if(state_changed)
         {
            ...
            local_static_tmp=4;            
            if(!lock_bi_d)
               pos_actu.state_bi=SBI_PRISE_G;   // met l'état du barilet en prise barilet
         }
      ...
      case SBG_PRISE_PAL_2:
         ...
         if(STATU_OVR_CPL(pos_actu.recept_MOT_BG))
         {
            ...
            if (prsg==CAPT_PRES)    // si il y a le vide
            {
               ...
               i=(i+2)%4;
               // ici i représente l'index du pallet sous le bras gauche
               pos_actu.pos_pallet[i%4]=B_NO_PAL;  // on elève le palet du tableau
               pos_actu.h_tower_g++;            
      ...
      case SBG_POSE_LT_2:
         ...
         if(STATU_OVR_CPL(pos_actu.recept_MOT_BG))
         {
            if((brasd_rdy==BRAS_RDY)&&(pos_actu.state_bd==SBD_POSE_LT_2))  // si l'autre bras est déjà près
            {
               // désactive le mode de démarrage automatique des commandes                           
               Spi_clr_cfg(MOT_START, MOT_GAUCHE);

               vang=VAN_OPEN;
               vand=VAN_OPEN;
               
               // envoie déjà l'ordre d'après
               ...
               {
                  pos_actu.nb_linteau--;
                  Spi_set_cfg(MOT_START, MOT_GAUCHE);   // démarre le mouvement
                  Spi_set_cfg(MOT_START, MOT_DROITE);   // démarre le mouvement
                  // checker ovr_cpl ou attendre fin a voir
      ...
      case SBG_POSE_LT_3:   
         ...
         {
            if(brasd_rdy==BRAS_RDY)  // si l'autre bras est déjà près
            {
               vang=VAN_CLOSE;
               vand=VAN_CLOSE;
   ...
   if(memo_state!=pos_actu.state_bg)      // test si la ms a changé d'état
      state_changed=1;
   else
      state_changed=0;
...
void  State_machine_bd()
{
   ...
   if(state_changed||(memo_state!=pos_actu.state_bd))      // test si une irq a changé l'état
      state_changed=1;
   else
      state_changed=0;
   ...
   switch(pos_actu.state_bd)
   {
      ...
      case SBD_POSE_LT_2:
         ...
         if(STATU_OVR_CPL(pos_actu.recept_MOT_BG))
         {
            if((brasg_rdy==BRAS_RDY)&&(pos_actu.state_bg==SBG_POSE_LT_2))  // si l'autre bras est déjà près
            {
               // désactive le mode de démarrage automatique des commandes                           
      ...
      case SBD_POSE_LT_3:        
         if(STATU_OVR_CPL(pos_actu.recept_MOT_BG)||STATU_POS_ATT(pos_actu.recept_MOT_BG))
         // le couple a été dépassé, on est en contacte avec le linteau
         {
            if(brasg_rdy==BRAS_RDY)  // si l'autre bras est déjà près
            {
               vang=VAN_CLOSE;
               vand=VAN_CLOSE;
   ...
   if(memo_state!=pos_actu.state_bd)      // test si la ms a changé d'état
      state_changed=1;
   else
      state_changed=0;
...
{
   // l'état est changé a l'extérieur de cet machine...
   static bit linteau_in=0;
   
   if(STATU_POS_ATT(pos_actu.recept_MOT_LT)) // si la position demandé est atteinte
   {
...
{
   if(STATU_POS_ATT(pos_actu.recept_MOT_BI)) // si la position demandé est atteinte
   {
      switch(pos_actu.state_bi)
      {
         case SBI_REPOT:
            ...
            else      // si il est plein je me prépart pour les bras
            {
         ...
         case SBI_ATT_PAL: // attend qu'un pallet arrive dans le barilet
            ...
            else if(pos_actu.pos_barillet&0x01) // le barrilet est fermé
            {
               pos_actu.pos_barillet=(pos_actu.pos_barillet+1)%8;
               Spi_send(MOT_BARILLET,BARILLET_OPEN1+(pos_actu.pos_barillet*BARILLET_DEMI_POS),V_MED,C_MED);            
            }
            else if  (pos_actu.pos_pallet[pos_actu.pos_barillet>>1]!=B_NO_PAL) // la place n'était pas vide
            {
               pos_actu.pos_barillet=(pos_actu.pos_barillet+2)%8;
               Spi_send(MOT_BARILLET,BARILLET_OPEN1+(pos_actu.pos_barillet*BARILLET_DEMI_POS),V_MED,C_MED);
            }
            else if((capav==CAPT_FULL)) // un pallet est entrée 
            {
               pos_actu.nb_pallet++;
               pos_actu.pos_barillet=(pos_actu.pos_barillet+2)%8;
               Spi_send(MOT_BARILLET,BARILLET_OPEN1+(pos_actu.pos_barillet*BARILLET_DEMI_POS),V_MED,C_MED);
         ...
         case SBI_PLEIN:   // le barilet est plein
            // attend qu'on passe a demande pallet -> demandé par state_machine_bx
         break;
         ...
         case  SBI_PRISE:     // les bras prennent les palet dans le barilet 
            if(!lock_bi_d&&!lock_bi_g) // attend que les bras enlève les vérous
               pos_actu.state_bi=SBI_REPOT;
         break;
