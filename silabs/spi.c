...
void Spi_send(uint8 motteur, uint16 angle, uint16 vitesse, uint16 couple)
{
   ...
   while (spi_state != SPI_FREE)
      ; // le dernier byte est dans le buffer
   while (SPI0CFG & 0x80)
      ; // il a �t� envoy�
   // enl�ve les cs
   NSS1 = 1;
   ...
   // si c'est au DSP qui g�re les bras � qui on veu parler
   if ((motteur == MOT_GAUCHE) || (motteur == MOT_DROITE))
   {
      spi_sel_DSP = SPI_DSP_BRAS;
      NSS2 = 0;
      if (motteur == MOT2BRAS) // si c'est le 2e motteur g�rer par le bras, d�cale de 3 case
         i = 6;
